 ...
 *
 * 该类主要包含了3步处理
 * 1.通过继承AbstractBeanDefinitionReader类，来使用ResourceLoader将资源文件路径转化为Resource
 * 2.通过DocumentLoader对Resource文件进行转化，将Resource文件转化为Document文件
 * 3.通过实现接口DefaultBeanDefinitionDocumentReader的BeanDefinitionDocumentReader类对Document文件进行解析
...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		//首先对resource资源文件使用EncodedResource进行封装，目的是考虑到Resource可能存在编码要求的情况
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			//从encodedResource中获取已经封装的Resource对象并从Resource中再次获取InputStream
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				//根据inputStream构造InputSource 它并不来自于spring，它的全路名：org.xml.sax
				InputSource inputSource = new InputSource(inputStream);
				if (encodedResource.getEncoding() != null) {
					inputSource.setEncoding(encodedResource.getEncoding());
				}
				//根据InputSource和Resource继续处理 进入逻辑的核心部分
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		/**
		 * 获取Document文件，委托给DocumentLoader类去执行Document的加载
		 * DocumentLoader是个接口,真正调用的是DefaultDocumentLoader
		 */
		return this.documentLoader.loadDocument(inputSource, getEntityResolver(), this.errorHandler,
				//getValidationModeForResource()方法用来获取对应资源的验证模式
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			//委托给XmlValidationModeDetector类进行自动检测验证模式
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		//加载及注册Bean的功能委托给BeanDefinitionDocumentReader接口的DefaultBeanDefinitionDocumentReader实现类处理
